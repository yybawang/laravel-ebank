...
export default (props) => {
    ...
    async function init(reset) {
        let p = reset ? 1 : page;
        let res = await axios.get('/identities', {params: {name, page: p}});
        setList(res);
    }
    ...
    return (
        <div>
            <div className={'head-filters py-2'}>
                ...
                <div className={'add'}><ButtonToolbar><Button variant={"primary"} onClick={()=> history.push('/identities/edit/0')}>添加</Button></ButtonToolbar></div>
            </div>
            <Table striped bordered hover>
                ...
                {list.data.map(row => <tr key={row.id}>
                    ...
                    <td>
                        <ButtonGroup size={"sm"}>
                            <Button onClick={()=> history.push('/identities/edit/'+row.id)}>编辑</Button>
