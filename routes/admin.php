...
Route::middleware([AdminUser::class])->namespace('Admin')->group(function(){
	...
	Route::prefix('index')->name('admin.基本信息管理.')->group(function(){
		...
		Route::get('sum_today','IndexController@sum_today');
		Route::get('sum_yesterday','IndexController@sum_yesterday');
	...
	Route::prefix('pay')->name('admin.支付秘钥管理.')->group(function(){
		...
		Route::get('collect','CollectController@index')->name('menu.个人/商家固码');
		Route::post('collect','CollectController@add');
		Route::delete('collect','CollectController@delete');
		Route::get('collect/{id}','CollectController@detail');
	});
